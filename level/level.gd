...
func _ready():
	if Settings.gi_quality == Settings.GIQuality.HIGH:
		...
		ProjectSettings["rendering/sdfgi/probe_ray_count"] = 2
	elif Settings.gi_quality == Settings.GIQuality.LOW:
		world_environment.environment.sdfgi_enabled = true
		ProjectSettings["rendering/sdfgi/probe_ray_count"] = 1
	else:
	...
	if Settings.ssao_quality == Settings.SSAOQuality.HIGH:
		...
		ProjectSettings["rendering/quality/ssao/quality"] = RenderingServer.ENV_SSAO_QUALITY_HIGH
	elif Settings.ssao_quality == Settings.SSAOQuality.LOW:
		world_environment.environment.ssao_enabled = true
		ProjectSettings["rendering/quality/ssao/quality"] = RenderingServer.ENV_SSAO_QUALITY_LOW
	else:
		world_environment.environment.ssao_enabled = false

	if Settings.bloom_quality == Settings.BloomQuality.HIGH:
		world_environment.environment.glow_enabled = true
		ProjectSettings["rendering/quality/glow/use_high_quality"] = true
	elif Settings.bloom_quality == Settings.BloomQuality.LOW:
		world_environment.environment.glow_enabled = true
		ProjectSettings["rendering/quality/glow/use_high_quality"] = false
	else:
	...
	if Settings.resolution == Settings.Resolution.NATIVE:
		...
		DisplayServer.window_set_size(minsize)
